        ...
        <table className="w-full">
          ...
          <tbody>
            {pages?.map((page, index) => (
              <tr onClick={() => navigate(`/content/page/${page.id}`)} key={index} className="border-b hover:bg-gray-50 cursor-pointer">
                <td className="py-3 px-4">{page.title.slice(0, 30)}</td>
                <td className="py-3 px-4">{page.sharedTimes}</td>
                <td className="py-3 px-4">{page.lastShared}</td>
              </tr>
            ))}
          </tbody>
        </table>
